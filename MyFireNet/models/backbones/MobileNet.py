...
class MobileNet(nn.Module):
    ...
    def freeze_stage(self, stage):
        if self.mobilenet_type == "mobilenet_v2":
            if stage >= 0:
                self.model.features[0][1].eval()
                for m in [self.model.features[0][0], self.model.features[0][1]]:
                    for param in m.parameters():
                        param.requires_grad = False
            for i in range(5, stage + 5):
                layer = getattr(self.model.features, '{}'.format(i))
                layer.eval()
            ...
            if stage >= 0:
                self.model.features[0][1].eval()
                for m in [self.model.features[0][1], self.model.features[0][1]]:
                    for param in m.parameters():
                        param.requires_grad = False
            for i in range(5, stage + 5):
                layer = getattr(self.model.features, '{}'.format(i))
...
if __name__ == "__main__":
    mobilenet = MobileNet(mobilenet_type="mobilenet_v3_small", pretrained=False)
    print(mobilenet)
    mobilenet.freeze_stage(1)
