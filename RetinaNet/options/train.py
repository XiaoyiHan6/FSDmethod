...
import yaml
import argparse
from data.collate import collater
from torchvision import transforms
from models.RetinaNet import RetinaNet
from torch.utils.data import DataLoader
from data.voc import VOC_ROOT, VocDetection
from data.augmentations import Normalizer, Augmenter, Resizer
...
class set_config(object):
    ...
    if cfg['Data']['name'] == 'VOC':
        ...
        dataset_val = VocDetection(cfg['Data']['root'], transform=transforms.Compose([Normalizer(), Resizer()]),
                                   set_name=[('2007', 'test')])
        ...
        dataset_val = FireDetection(cfg['Data']['root'],
        ...
        dataset_train = FireDetection(cfg['Data']['root'],
                                      transform=transforms.Compose([Normalizer(), Augmenter(), Resizer()]))
        dataset_val = FireDetection(cfg['Data']['root'], transform=transforms.Compose([Normalizer(), Resizer()]),
                                    set_name='test')
        ...
        if cfg['Data']['root'] != MY_FIRE_SMOKE_DATASET_ROOT:
            ...
            raise ValueError("WARNING: Using default mini MyFireDataset, but " +
                             "--dataset_root was not specified.")
        dataset_train = FireDetection(cfg['Data']['root'],
                                      transform=transforms.ComE_Fire_Smoke_Dataset_voc360.yamlpose(
                                          [Normalizer(), Augmenter(), Resizer(min_side=cfg['Data']['size'])]),
                                      set_name='minitrain')
        dataset_val = FireDetection(cfg['Data']['root'],
                                    transform=transforms.Compose([Normalizer(), Resizer(min_side=cfg['Data']['size'])]),
                                    set_name='minitest')
    ...
    model = RetinaNet(backbones_type=cfg['Backbones']['name'] + str(cfg['Backbones']['depth']),
