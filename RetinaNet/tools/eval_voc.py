...
from options.eval import args, cfg, dataset_eval, VOC_ROOT, retinanet_eval, VISIFIRE_VOC_ROOT, FIRESENSE_VOC_ROOT, \
    FURG_FIRE_DATASET_VOC_ROOT, BOWFIREDATASET_VOC_ROOT, FIRE_SMOKE_DATASET_VOC_ROOT, MY_FIRE_SMOKE_DATASET_ROOT, \
    TEST_ROOT
...
elif cfg['Data']['name'] == 'myfire' or cfg['Data']['name'] == 'minimyfire' or cfg['Data'][
...
def get_voc_results_file_template(image_set, cls):
    ...
    filedir = os.path.join(devkit_path, 'RetinaNet/{}/det'.format(cfg['Data']['name']))
    if not os.path.exists(filedir):
        os.makedirs(filedir)
...
def do_python_eval(output_dir='output', use_07=True):
    ...
    cachedir = os.path.join(devkit_path, 'RetinaNet/{}'.format(cfg['Data']['name']))
    aps = []
...
def eval_voc(dataset, model, threshold=0.05):
    ...
    output_dir = get_output_dir(devkit_path, 'RetinaNet/{}/annot_cache'.format(cfg['Data']['name']))
    ...
    aps = evaluate_detections(results, output_dir)
    return aps


def evaluate_detections(results, output_dir):
